...
from app import db
from app.models import PurchaseOrder, Supplier, RequestOrderItem
...
def get_delivery_maintenance_list():
    ...
    try:
        ...
        # Apply filters - PO number filter first for better performance
        if po_number_filter:
            logger.info(f"[SEARCH] Applying PO number filter: {po_number_filter}")
            # Use contains for more reliable filtering
            query = query.filter(PurchaseOrder.purchase_order_no.contains(po_number_filter))

        if supplier_region_filter:
            logger.info(f"[SEARCH] Applying supplier_region filter: {supplier_region_filter}")
            query = query.filter(Supplier.supplier_region == supplier_region_filter)

        if status_filter:
            logger.info(f"[SEARCH] Applying status filter: {status_filter}")
            query = query.filter(PurchaseOrder.delivery_status == status_filter)
        
        # Execute query
        pos = query.all()
        print(f"[DEBUG] Query returned {len(pos)} POs after filtering (po_number={po_number_filter})")
        logger.info(f"Query returned {len(pos)} POs after filtering (po_number={po_number_filter})")
        ...
        for po in pos:
            ...
            # Filter based on supplier region parameter (ensure we respect the filter)
            # If supplier_region_filter is specified, only include matching region
            if supplier_region_filter:
                if supplier.supplier_region != supplier_region_filter:
                    continue

            # Check if PO should be in delivery maintenance list
    ...
    try:
        ...
        for item in po.items.all():
            ...
            if item.source_detail_id:
                ...
                if req_item:
                    # Map delivery status to requisition item status
                    if new_status == 'shipped':
                        req_item.item_status = 'shipped'
                    elif new_status == 'delivered':
                        req_item.item_status = 'arrived'
                    elif new_status in ['foreign_customs', 'taiwan_customs', 'in_transit']:
                        req_item.item_status = 'shipped'  # Keep as shipped for transit statuses
