...
class RequestOrderItem(db.Model):
    ...
    reviewer_id = db.Column(db.Integer, db.ForeignKey('users.user_id'))  # 審核人ID
    reviewed_at = db.Column(db.DateTime)  # 審核時間
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    def question(self, reason, reviewer_id=None):
        """Mark the item as questioned with reviewer tracking"""
        ...
        if self.reviewer_id:
            ...
            if reviewer:
                reviewer_info = {
                    ...
                    'chinese_name': reviewer.chinese_name
                }
